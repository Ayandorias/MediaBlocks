 ...
 *
 * Loads the actuyl configuration of the device
 *
 ...
 * @brief Destructor
 *
 * The destructor of the configuration class.
 */
 ...
 *
 * Creates a new room on the device with the given information by obj.
 * The object for the new room should contain the following information to create the room.
 * @code
 * { "roomname":"Office" }
 * @endcode
 *
 ...
 *
 * @return On success: The new created room
 * @code {"id": 0, "name": "Office", "port": 6001} @endcode
 * @return Room available:
 * @code { "code": 1, "message" : "Resource available!"} @endcode
 * @return If the parameter in the object is not "roomname", the following JSON-object will be returned.
 * @code { "code": 4, "message" : "Request is not available!"} @endcode
 *
 *
 * @test MediaBlocks::Test::_Configuration::createNewRoom()
 */
 ...
 *
 * @return The list of rooms setDeviceName(QJsonObject &obj)
 *
 * @test This function is tested with MediaBlocks::Test::_Configuration::checkAllRooms()
 */
...
QJsonDocument MediaBlocks::Configuration::setFactoryReset() {
	...
	pl.insert("enable", QJsonValue(true));
	pl.insert("playlist_dir", QJsonValue("<path-to-your-playlists>"));
	device.insert("m3u", QJsonValue(pl));

	local.insert("enbale", QJsonValue(true));
	local.insert("media_dir", QJsonValue("<path-to-your-music>"));
	local.insert("library", QJsonValue("sqlite"));
	device.insert("local", QJsonValue(local));
 ...
 * If saving the current configuration was succesful the signal void changed() is emitted.
 *
 * @return True if successful, false otherwise.
 */
